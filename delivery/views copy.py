...
def confirm_product(request):
    if request.method == "POST":
        ...
        try:
            ...
            vendor_user = product.user  # Vendor info
            order = item.order           # CartOrder instance (customer info)
            order_user = order.user      # Customer user object

            # Send email to delivery provider
            send_mail(
